...
// ✅ Permanently delete the authenticated job seeker's own company review
// DELETE /api/companies/verified/:id/reviews/:reviewId
exports.deleteCompanyReview = async (req, res) => {
  try {
    ...
    const reviews = Array.isArray(company.employerProfile?.reviews)
      ? company.employerProfile.reviews
      : [];
    ...
    company.employerProfile.reviews.splice(reviewIndex, 1);
    company.markModified('employerProfile.reviews');
    await company.save();
    ...
    console.error('Error permanently deleting company review:', error);

    if (error?.name === 'CastError') {
      return res.status(404).json({
        success: false,
        message: 'Company or review not found.',
      });
    }
