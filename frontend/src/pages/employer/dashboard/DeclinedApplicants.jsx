...
const DeclinedApplicants = () => {
  ...
  const fetchDeclinedApplications = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const res = await axios.get('http://localhost:5000/api/applications/employer/declined', {
        headers: getAuthHeaders(),
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await axios.patch(
        `http://localhost:5000/api/applications/${deleteTarget._id}/archive-declined`,
        {},
        { headers: getAuthHeaders() }
      );
