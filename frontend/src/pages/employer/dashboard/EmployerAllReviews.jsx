...
const ReviewerAvatar = ({ src, name }) => {
  ...
  return <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border border-[#dfe7f0] bg-[#eef5fc]">{src && !failed ? <img src={src} alt={`${name || 'Reviewer'} profile`} className="h-full w-full object-cover" onError={() => setFailed(true)} /> : <span className="text-sm font-bold text-[#2e66a6]">{initial}</span>}</div>;
};
