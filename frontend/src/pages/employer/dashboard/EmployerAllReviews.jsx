...
const EmployerAllReviews = () => {
  ...
  useEffect(() => {
    ...
    api.get('/auth/me').then((response) => {
      ...
      setReviews(Array.isArray(profile.reviews) ? profile.reviews : []);
    }).catch((requestError) => {
    ...
    <EmployerLayout>
      <main className="min-h-screen px-1 py-8">
        <div className="mx-auto w-full max-w-7xl">
          <button type="button" onClick={() => navigate('/employer/company-profile', { state: { activeTab: 'reviews' } })} className="mb-5 inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50"><span aria-hidden="true">←</span> Back</button>
          <section className="rounded-[18px] border border-[#d1d5db] bg-white p-5 shadow-[0_2px_6px_rgba(15,23,42,0.05)] sm:p-7">
            ...
            {loading ? <p className="py-16 text-center text-black/60">Loading reviews...</p> : error ? <p className="py-16 text-center text-red-600">{error}</p> : <>
              <div className="mt-7 grid grid-cols-1 gap-5 rounded-2xl border border-[#dfe7f0] bg-[#fbfcfe] p-5 sm:grid-cols-[170px_minmax(0,1fr)] sm:items-center sm:p-6">
                <div className="text-center sm:border-r sm:border-[#dfe7f0] sm:pr-5"><p className="text-4xl font-bold leading-none text-[#27364a]">{summary.rating.toFixed(1)}</p><div className="mt-2 flex justify-center gap-0.5">{[1, 2, 3, 4, 5].map((star) => <span key={star} className={`text-lg ${star <= Math.round(summary.rating) ? 'text-[#f2b313]' : 'text-[#d9e0e8]'}`}>★</span>)}</div><p className="mt-1 text-sm text-black/65">{summary.count} ratings in total</p></div>
                <div className="space-y-2">{[5, 4, 3, 2, 1].map((star) => { const count = summary.breakdown[star]; const percent = summary.count ? (count / summary.count) * 100 : 0; return <div key={star} className="grid grid-cols-[18px_minmax(0,1fr)_28px] items-center gap-2"><span className="text-sm font-medium text-black/70">{star}</span><div className="h-2 overflow-hidden rounded-full bg-[#e9edf2]"><div className="h-full rounded-full bg-[#f2b313]" style={{ width: `${percent}%` }} /></div><span className="text-right text-sm text-black/65">{count}</span></div>; })}</div>
              </div>
              <div className="mt-7 space-y-5">{visibleReviews.length === 0 ? <div className="rounded-2xl border border-dashed border-[#d8e2ee] px-6 py-14 text-center text-black/55">{reviews.length ? 'No reviews match your search.' : 'No application reviews yet.'}</div> : visibleReviews.map((review, index) => <article key={review._id || `${safePage}-${index}`} className="rounded-2xl border border-[#dfe7f0] bg-white px-5 py-5 shadow-[0_10px_28px_rgba(46,102,166,0.06)] sm:px-6 sm:py-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-[17px] font-bold text-black">{review.reviewerName || 'Anonymous User'}</h2><p className="mt-1 text-sm text-black/55">{review.roleAppliedFor || 'Role not provided'}{formatTimeAgo(review.createdAt) ? ` · ${formatTimeAgo(review.createdAt)}` : ''}</p></div><span className="rounded-full border border-[#dfe7f0] bg-[#fbfcfe] px-3 py-1 text-xs font-semibold text-[#2e66a6]">{outcomeLabel(review.outcome)}</span></div><p className="mt-5 whitespace-pre-line text-[16px] leading-7 text-black/80">{review.message || 'No written review provided.'}</p><div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="First reply" value={review.daysToFirstResponse == null ? 'Not provided' : `${Number(review.daysToFirstResponse) || 0}d`} /><Metric label="Total length" value={review.totalProcessDays == null ? 'Not provided' : `${Number(review.totalProcessDays) || 0}d`} /><Metric label="Process" value={`${Number(review.processRating ?? review.rating) || 0}/5`} /><Metric label="Apply again?" value={typeof review.wouldApplyAgain === 'boolean' ? (review.wouldApplyAgain ? 'Yes' : 'No') : 'Not provided'} /></div></article>)}</div>
