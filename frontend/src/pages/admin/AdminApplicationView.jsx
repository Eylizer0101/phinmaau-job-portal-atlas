...
const Icon = ({ name, className = "h-4 w-4", ...props }) => {
  ...
  const icons = {
    ...
    external: <><path strokeLinecap="round" strokeLinejoin="round" d="M14 3h7v7" /><path strokeLinecap="round" strokeLinejoin="round" d="M10 14L21 3" /><path strokeLinecap="round" strokeLinejoin="round" d="M21 14v6a2 2 0 01-2 2H5a2 2 0 012-2V7a2 2 0 012-2h6" /></>,
    globe: <><path strokeLinecap="round" strokeLinejoin="round" d="M3 12h18M12 3c3.5 4 3.5 14 0 18m0-18c-3.5 4-3.5 14 0 18M4.5 7.5h15M4.5 16.5h15" /><circle cx="12" cy="12" r="9" /></>,
  };
...
const AdminApplicationView = () => {
  ...
  const submittedResumeSnapshot = application?.resumeSnapshot || null;
  const hasSubmittedResumeSnapshot = Boolean(
    submittedResumeSnapshot &&
      typeof submittedResumeSnapshot === "object" &&
      Object.keys(submittedResumeSnapshot).length
  );
  ...
  const handleViewSubmittedResume = () => {
    ...
    sessionStorage.setItem(
      ...
      JSON.stringify({
        ...
        workExperiences:
          ...
          snapshotFormData.workExperiences ||
    ...
    const jobseekerId = jobseeker?._id || jobseeker?.id || "preview";
    navigate(`/admin/users/${jobseekerId}/resume-preview`);
  };
          ...
          <div className="-mt-10 sm:-mt-12 relative z-10">
            <section className={`${UI.card} mb-6 min-h-[210px] p-5 sm:min-h-[225px] sm:p-7 lg:min-h-[240px] lg:p-8 flex items-center`}>
              <div className="flex w-full flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <span className={UI.chip}>
                        <Icon name="briefcase" className="h-3.5 w-3.5 text-black/60" />
                        {job.jobType || "Employment type not specified"}
                      </span>

                      <span className={UI.chip}>
                        <MyApplicationsSvgIcon name="laptop" className="h-3.5 w-3.5 text-black/60" />
                      ...
                      <span className={UI.chip}>
                        <Icon name="mapPin" className="h-3.5 w-3.5 text-black/60" />
                ...
                {hasSubmittedResumeSnapshot ? (
                  <div className="flex w-full shrink-0 lg:w-[260px]">
                    <button
                      type="button"
                      onClick={handleViewSubmittedResume}
                      className={cn(
                        "inline-flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-[#2e66a6] px-5 text-base font-semibold text-white shadow-[0_10px_22px_rgba(46,102,166,0.22)] transition hover:bg-[#25578f]",
                        UI.ring
                      )}
                    >
            ...
            <div className="space-y-5">
              <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <MetricCard
                  ...
                  value={job.educationLevel || job.educationalRequirements || "Educational requirement not specified"}
                />
                <MetricCard
                  icon="globe"
              ...
              <section className={UI.sectionCard}>
                <div className={UI.insetBody}>
                  ...
                  <div className="mt-4 text-sm leading-relaxed text-black/70 sm:text-base">
                    <RichTextContent value={job.requirements || job.qualification} fallback="No requirements provided." />
                  </div>
                </div>
              </section>
              ...
              <section className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <div className={UI.sectionCard}>
                  ...
                  <div className="border-t border-[#e6edf5] px-4 py-3">
                    <a
                      href={buildOpenStreetMapUrl({ coords: getJobCoordinates(job), address: location })}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="rounded text-xs font-medium text-[#2e66a6] hover:underline"
                      title="Open work location in OpenStreetMap"
                    >
                      {location || "Work address not specified"}
                    </a>
                  </div>
                </div>
              </section>
