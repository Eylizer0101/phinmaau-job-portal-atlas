...
const companyNameOf = (item = {}) => item.job?.companyName || item.employer?.employerProfile?.companyName || 'Company';
const companyIndustryOf = (item = {}) => item.job?.industry || item.job?.category || item.employer?.employerProfile?.industry || 'Industry not specified';
...
}[status] || 'border-slate-300 bg-slate-50 text-slate-600');
...
export default function AdminJobseekerRequestHistory() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 py-8">
    ...
    <section className="overflow-x-auto rounded-2xl border bg-white shadow-sm"><table className="w-full min-w-[900px]"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr>{['Request Date','Company','Job Title','Request Type','Status','Action'].map(label=><th key={label} className="px-6 py-5">{label}</th>)}</tr></thead><tbody className="divide-y">{!loading && filtered.map(item=>{ const requestStatus=String(item.employmentStatusRequest?.status || 'pending').toLowerCase(); const companyLogo=companyLogoOf(item); const companyName=companyNameOf(item); return <tr key={item._id}><td className="px-6 py-5 text-sm">{formatDate(item.employmentStatusRequest?.requestedAt)}</td><td className="px-6 py-5"><div className="flex items-center gap-3">{companyLogo ? <img src={companyLogo} alt={`${companyName} logo`} className="h-11 w-11 shrink-0 rounded-xl border border-slate-200 object-cover"/> : <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#2e66a6] text-xs font-bold text-white">{companyName.slice(0,2).toUpperCase()}</span>}<div className="min-w-0"><p className="truncate font-semibold text-slate-950">{companyName}</p><p className="mt-1 truncate text-xs font-normal text-slate-500">{companyIndustryOf(item)}</p></div></div></td><td className="px-6 py-5 font-semibold">{item.job?.title || '—'}</td><td className="px-6 py-5">{reasonLabel(item.employmentStatusRequest?.reason)}</td><td className="px-6 py-5"><span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold uppercase ${statusBadgeClass(requestStatus)}`}>{requestStatus.replace('_',' ')}</span></td><td className="px-6 py-5"><button onClick={()=>navigate(`/admin/jobseeker-status-requests/${jobseekerId}/${item._id}`)} className="rounded-xl border p-3 text-[#2e66a6]"><Eye size={18}/></button></td></tr>;})}{!loading && !filtered.length && <tr><td colSpan="6" className="p-12 text-center text-slate-500">No requests found.</td></tr>}</tbody></table></section>
