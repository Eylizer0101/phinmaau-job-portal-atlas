...
import { ArrowLeft, BriefcaseBusiness, Building2, CheckCircle2, Eye, EyeOff, LockKeyhole, Mail, Pencil, Phone, Upload, UserRound, X } from "lucide-react";
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm sm:p-9">
          ...
          <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50/70 p-5"><p className="font-bold text-[#173b78]">Password Requirements</p><div className="mt-3 space-y-2">{requirements.map((item) => <p key={item.label} className={`flex items-center gap-2 text-sm ${item.valid ? "text-emerald-700" : "text-slate-700"}`}><CheckCircle2 size={17} className={item.valid ? "fill-emerald-600 text-white" : "fill-[#173b78] text-white"} />{item.label}</p>)}</div></div>
